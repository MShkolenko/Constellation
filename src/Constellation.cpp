    ...
    void BehaveTick(Companion& c, uint32 diff)
    {
        ...
        switch (c.Mode)
        {
            ...
            case Behavior::Gathering:
            {
                ...
                SpellInfo const* lockSpell = go->GetSpellForLock(self);
                // ТОЛЬКО МГНОВЕННОЕ ЗАКЛИНАНИЕ ЗАМКА (Кодекс, задача 134).
                //
                // Читаемое или канальное открывается не в этот такт, а весь учёт вокруг —
                // добыча, снимок квестов, трата точки — считается сразу после отправки. Первая
                // редакция пыталась это обойти возвратом «посчитаю следующим заходом», и
                // Кодекс показал, что возврата не существует: при повторном входе код просто
                // кастует снова, потеряв исходный снимок. Держать полноценную стадию ожидания
                // ради случая, которого у здешних замков нет, дороже, чем его исключить.
                // Не мгновенное — идём прежним путём, пакетом использования; хуже прежнего не
                // станет, потому что прежде так открывалось вообще всё.
                if (lockSpell && (lockSpell->CalcCastTime() > 0 || lockSpell->IsChanneled()))
                {
                    TC_LOG_INFO("server.worldserver",
                        "Constellation СБОР {}: у {} ({}) заклинание замка {} не мгновенное — открываю пакетом",
                ...
                bool const interacted = lockSpell ? stateMoved : wasReady;
