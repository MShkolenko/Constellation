...
struct Settings
{
    ...
    // КАКИМ СТРАТЕГИЯМ ДВИЖКА ПОЗВОЛЕНО РЕШАТЬ. Ноль значит «ни одной»: даже при включённом
    // `Engine` шов остаётся немым, потому что ставить будет некому. Это второй замок, и он
    // независим от первого — включение должно быть решением из двух шагов.
    //
    // МАСКА, А НЕ СПИСОК: она уже есть и уже фильтрует ставки. Разбор просил включать по
    // одному классу действий ГЛОБАЛЬНО для всего состава — маска делает ровно это, а состав
...
struct Companion
{
    ...
    // ГДЕ Я ПОБЕЖДАЛ: та же клетка 50x50. Нужна ровно для одного — отличить место, которое
    // меня только убивает, от места трудного, но проходимого. Замер (запись 61): семеро
    // рыцарей смерти дали 31 гибель и НОЛЬ побед в своём анклаве, а Garrick — 12 побед при
    // 8 гибелях в одной зоне. Запрет по месту без этой поправки одинаково закрыл бы обоих,
    // и второму он стоил бы всей его работы.
    std::unordered_map<uint64, uint16> KillSpots;
    ...
    // ЗДЕСЬ СТОЯЛА ОТСЕЧКА ПО ВИДУ ОБЪЕКТА, И ОНА СНЯТА (оператор, 2026-09-05).
    //
    // Записи 64 и 65 закрывали вид целиком, когда его точки «ничего не дали». Посылка была
    // неверна. Дверь квеста 14098 — не отказ, а ЛОТЕРЕЯ: 67869 «Стук» случайно выбирает один
    // из двух своих эффектов, и выбегает либо мирный житель (зачёт существа 35830), либо
    // ворген (ничего). Считать половину нормальных исходов уликой значит выключать работающую
    // механику; обход четырнадцати дверей обрывался на шестой.
    //
    // Заменено правилом, которое назвал оператор и которое подтверждает содержание квеста:
    // точка ОДНОРАЗОВАЯ для персонажа. Один готовый клик тратит точку, дальше идём к
    // следующей. Обход конечен сам по себе, поэтому отдельный предел по виду не нужен —
    // ни DeadEntry, ни DeadCounted, ни DeadEntryUses, ни DeadEntryNoted, ни DeadQuestSet,
    // существовавший только чтобы их сбрасывать.
    // ОЖИДАНИЕ КАСТА ЗАМКА. Пока заклинание читается, судить исход нечем: окно добычи не
    // открыто, цели не сдвинулись. Поэтому спутник остаётся на точке, а снимки, взятые ДО
    // отправки, живут здесь — брать их заново после каста значило бы сравнивать мир с самим
    // собой и никогда не увидеть изменения (Кодекс, задачи 134-136).
    uint32 LockCastSpawn = 0;               // на какой точке ждём
    uint32 LockCastSpace = 0;               // место в сумках до отправки
    // МОМЕНТ НАЧАЛА ОЖИДАНИЯ, А НЕ СРОК ЕГО КОНЦА (Кодекс, задача 138). Абсолютный срок в
    // uint32 сравнивался через «меньше» и на переполнении миллисекундного счётчика давал
    // мгновенный ложный тайм-аут. Беззнаковая разность «сейчас минус начало» переполнение
    // переживает правильно.
    uint32 LockCastStartMs = 0;
    uint32 LockCastSpellId = 0;             // что именно читаем — чтобы отменить адресно
    ObjectGuid LockCastId;                  // идентификатор каста, как его слепил клиент
    bool LockCastReady = false;             // было ли состояние объекта готовым до отправки
    std::vector<std::pair<uint32, int32>> LockCastSnap;   // снимок квестов до отправки
...
// ОБЁРТКА ДЛЯ ЛЕСТНИЦЫ: подпись прежняя, поэтому ни одно место вызова не тронуто.
    ...
    void BehaveTick(Companion& c, uint32 diff)
    {
        ...
        if (c.BookDiagMs)
            c.BookDiagMs = (c.BookDiagMs <= diff) ? 0 : c.BookDiagMs - diff;
        if (c.SeekRecheckMs)
            c.SeekRecheckMs = (c.SeekRecheckMs <= diff) ? 0 : c.SeekRecheckMs - diff;
        ...
        // СЛОМАН — ЧИНИТЬСЯ ИЗ ЛЮБОГО СОСТОЯНИЯ, А НЕ ТОЛЬКО ИЗ «СТОЮ».
        //
        // В прошлый раз было починено «что запрещает бой, то и посылает чиниться»: предикат
        // стал общим. Но МЕСТО осталось прежним — весь блок торговли лежит внутри
        // `case Behavior::Idle`, и пятисекундный таймер `VendScanMs` тикает тоже только там.
        // Значит спутник, которому всегда есть чем заняться, не вспоминает о ремонте НИКОГДА:
        // предикат один, а спрашивают его в одном состоянии из двенадцати.
        //
        // Замер (круг 70, 5 минут мира): Thragan — 6915 медяков, пять сломанных вещей и НОЛЬ
        // входов в «стою» за всё время; Wachiwi (822 медяка, 7 сломанных) и Chayton (712, 6) —
        // тоже ноль. Для сравнения, спутники без работы входят в «стою» по 28-29 раз.
        //
        // ЧИСЛА ЗДЕСЬ ПОСЧИТАНЫ ЯДРОМ, А НЕ БАЗОЙ, и это не придирка. Первая редакция этого
        // комментария называла 140 сломанных вещей у 38 спутников и вела примером Garrick с
        // девятью. Счёт был по `item_instance.durability = 0`, а у шеи, колец, аксессуаров,
        // плаща, рубахи и накидки прочности НЕ БЫВАЕТ — там всегда ноль, и колонки максимальной
        // прочности в базе нет вовсе. `Item::IsBroken()` — это «прочность 0 И максимальная
        // больше нуля», поэтому честный счёт даёт 52 вещи у 10 спутников, ровно как в строках
        // самого модуля. Персонаж оператора, показавший «четыре сломанных», на деле цел, а
        // Garrick к моменту правки успел починиться сам.
        //
        // Поэтому: сломан — становимся «стою» и обнуляем таймер, а дальше решает та же самая
        // проверенная логика похода, которую мы не трогаем.
        //
        // ОТКУДА НЕ ДЁРГАЕМ, И ПОЧЕМУ ИМЕННО ОТТУДА. Первая редакция этого сторожа исходила из
        // того, что блок торговли в «стою» идёт ПЕРВЫМ. Это неправда, и Кодекс (задача 115)
        // показал цену: раньше него в «стою» делаются обновки, выход из боя, обработка области,
        // СДАЧА ГОТОВОГО КВЕСТА и отдых. Готовый квест уводит в `TurningIn`, откуда сторож
        // вернул бы в «стою», а «стою» снова в `TurningIn` — вечные качели, без сдачи и без
        // ремонта.
        //
        // Значит исключаем всё, где работа уже НАЧАТА: сдачу квеста (доказанный цикл и прямое
        // нарушение порядка «сначала сдать, потом чиниться»), сбор и взаимодействие (посланный
        // опкод и ожидание зачёта по нему), отдых и труп, полёт и саму торговлю.
        //
        // И САМ БОЙ — по состоянию, а не только по `IsInCombat()` (Кодекс, задача 116). Ядро
        // снимает признак боя в момент смерти цели, а состояние `Attacking` в этот миг ещё не
        // разобрало исход: не засчитало победу, не записало её в `KillSpots`, не собрало добычу.
        // Сторож успел бы вклиниться между этими двумя мгновениями и украсть у нас ровно ту
        // запись, ради которой ставился прошлый круг.
        //
        // Остаются `Travelling`, `ApproachingTarget`, `SeekingGiver` и `FollowingOwner` — их
        // прерывание стоит одного повторного поиска, и ничего не теряет: опкода они ещё не
        // посылали, а бой в `ApproachingTarget` начинается только вместе с переходом в
        // `Attacking`, после успешного `TryAttack`.
        //
        // Порядок условий тоже не случаен (Кодекс): дешёвые проверки состояния и боя стоят
        // ПЕРЕД `BrokenCount`, который обходит девятнадцать слотов. `BrokenCount`, а не
        // `BrokenForFight`, намеренно: второй печатает строку «не идёт в бой», и звать его
        // отсюда значило бы говорить о бое там, где боя нет.
        if (Cfg().Vending && !c.VendCooldownMs
            && c.Mode != Behavior::Idle && c.Mode != Behavior::Vending
            && c.Mode != Behavior::Recovering && c.Mode != Behavior::TakingFlight
            && c.Mode != Behavior::TurningIn && c.Mode != Behavior::Gathering
            && c.Mode != Behavior::Talking && c.Mode != Behavior::Attacking
            && !self->IsInCombat() && !self->IsInFlight()
            && BrokenCount(self) > 0)
        {
            c.VendScanMs = 0;               // решать сразу, а не через пять секунд
            Switch(c, self, Behavior::Idle, "сломано — иду чиниться");
            return;
        }
        ...
        switch (c.Mode)
        {
            case Behavior::Idle:
            {
                ...
                // ШОВ РЕШЕНИЯ: ДВИЖОК ХОДИТ ПЕРВЫМ, И ЭТО ЕДИНСТВЕННОЕ МЕСТО, ГДЕ ОН РЕШАЕТ.
                //
                // ВНУТРИ `Idle`, А НЕ ПЕРЕД `switch`: перед ним движок вмешивался бы и в
                // `Talking`, и в `Attacking`, у которых своё намерение и своё состояние — тот
                // самый «жизненный цикл намерения», о котором предупредил разбор.
                //
                // И ИМЕННО ЗДЕСЬ, А НЕ НИЖЕ. Первая редакция стояла перед проверкой отдыха — то
                // есть ПОСЛЕ отхода из боя и после `FindTurnIn`, которые сами переключают режим.
                // Лестница решала первой, и сдача движка не выиграла бы никогда: смысл шва
                // отменялся его положением.
                //
                // ГРАНИЦА — ТАМ, ГДЕ КОНЧАЕТСЯ ОБСЛУЖИВАНИЕ И НАЧИНАЮТСЯ РЕШЕНИЯ. Выше: дроссель
                // `idleScan`, который обязан тикать всегда, и обновки, про которые их собственный
                // комментарий говорит «ничего не переключает, просто улучшает всё, что будет
                // дальше». Ниже: ветки, которые ПЕРЕКЛЮЧАЮТ режим, — их движок либо перебивает
                // своей ставкой, либо пропускает, не поставив ни одной.
                //
                // ОТХОД ИЗ БОЯ ОТ ЭТОГО НЕ СЛАБЕЕТ, хотя он теперь ниже. Спутник в бою со
                // сломанным оружием не даст движку ни одной ставки: отдых в бою невозможен по
                // предикату, а обход целей у сломанного не запускается вовсе — `BrokenForFight`
                // стоит первой его проверкой. Значит такт вернётся лестнице, и она отойдёт. Это
                // следствие кода, а не надежда.
                //
                // «ВЗЯЛСЯ» — НЕ «ВЫШЛО». Ход возвращается лестнице ТОЛЬКО при `Idle`: действие
                // вправе написать в мир и потом вернуть ложь, и отдать ему ход после этого
                // значило бы получить двух управляющих внутри одного такта.
                //
                // ВЫКЛЮЧЕНО ДВАЖДЫ: `Cfg().Engine` ложь по умолчанию, и маска стратегий — ноль,
                // так что даже включённый флаг не даёт поставить ни одной ставки.
                // ВЫКЛЮЧЕНИЕ ЗАБЫВАЕТ, А НЕ ЗАМОРАЖИВАЕТ. Снятый флаг останавливает такт и сам
                // по себе не трогает состояние: отсрочки, прогресс дороги и счёт отдыха пережили
                // бы выключение, и при повторном включении движок продолжил бы с памятью о мире,
                // которого уже нет — за это время спутник ходил под лестницей, гиб, менял карту.
                // Смена эпохи такое чистит, но флаг эпоху не двигает (разбор).
                //
                // Один раз на переход: `HasTicked` показывает, что состояние заводилось, и
                // обнуляется вместе со всем прочим, так что второй раз условие не сработает.
                ...
                if (self->IsInCombat() && BrokenForFight(c, self) && !c.FleePauseMs)
                {
                    ...
                    if (!nearest)
                    {
                        ...
                        //
                        // ПРИБОР, А НЕ ПРАВКА (спецификация движка v2 §6′). Это единственная
                        // запись в мир во всём модуле, у которой нет клиентского пакета:
                        // клиент не посылает «сбрось мне бой». Замена — CMSG_ATTACK_STOP и
                        // уйти из привязки, чтобы ядро сбросило бой само, — это СМЕНА
                        // ПОВЕДЕНИЯ, и её нельзя вносить, не зная цены нынешнего. Строка
                        // отвечает на два вопроса: как часто и после какого бегства.
                        TC_LOG_INFO("server.worldserver",
                            "Constellation БОЙ-ПУСТОЙ {}: в бою без нападающих — сбрасываю"
                            " (бегство {} мс, всего {} мс, точка {})",
                            self->GetName(), c.FleeMs, c.FleeTotalMs, c.FleeHasPoint ? 1 : 0);
                ...
                {
                    // ДАЛЕКО — СНАЧАЛА ВОЗДУХОМ, как это уже делают дорога к цели задания и
                    // поход к квестодателю. Без этого спутник идёт пешком к принимающему за
                    // шесть тысяч ярдов, построитель отвечает «пути нет» на первом же шаге,
                    // и круг повторяется каждые пять минут вечно (замер 2026-09-04: пятнадцать
                    // отказов, у всех «упёрлись», тип пути A, пройдено ноль ярдов).
                    // Только к принимающему НА КАРТЕ: сдача самому себе никуда не идёт.
                    if (c.TurnInEntry && Cfg().Flying)
                    {
                        if (HearthTowards(c, self, c.TurnInPos))
                            return;             // камень уносит домой, дорога продолжится оттуда
                        if (PlanFlight(c, self, c.TurnInPos))
                        {
                            Switch(c, self, Behavior::TakingFlight, "лечу сдавать квест");
                            return;
                        }
                    }
                ...
                else if (Cfg().Vending && !c.VendCooldownMs)
                {
                    ...
                    // ЧТО ЗАПРЕЩАЕТ БОЙ, ТО И ПОСЫЛАЕТ ЧИНИТЬСЯ — ОДИН ПРЕДИКАТ НА ОБА КОНЦА.
                    //
                    // Здесь стояло «CannotFight && сломано > 0», то есть поход начинался только
                    // при ПУСТОЙ РУКЕ. А от боя отстраняет BrokenForFight, которому довольно
                    // ЛЮБОЙ сломанной вещи. Спутник со сломанной бронёй и целым оружием попадал
                    // между двумя условиями: драться не может, чиниться не идёт — и стоит так,
                    // пока кто-нибудь не починит его командой. Замер: 46 спутников из 122 со
                    // сломанным (в среднем 3.1 вещи, максимум 9), 17 отказов идти в бой и НОЛЬ
                    // походов к ремонтнику за всё время наблюдения.
                    //
                    // Оговорка про рыцарей смерти сохраняется сама собой: она была не про
        ...
        if (FindGiverByMap(self, mem, &c.SeekEntry, &c.SeekSpawn, &c.SeekPos, &qid))
            ...
            case Behavior::SeekingGiver:
            {
                ...
                // ОГЛЯДЫВАЕМСЯ НА ХОДУ, А НЕ ТОЛЬКО ПО ПРИХОДУ.
                //
                // Оператор, 2026-09-06: «есть знак — идём и спрашиваем что предлагает».
                // Круг 71 снял вето на выбор, но выбор всё равно не менялся: замер дал
                // десять походов по карте средней длиной 228 ярдов, восемь из них дальше
                // ста пятидесяти, и ВСЕ десять начались со слов «в обзоре никто ничего не
                // предлагает». Дошли тоже все десять — походы не срываются. Срывается
                // другое: у Дэверела отбор нашёл Marshal McBride в ЧЕТЫРЁХ ярдах, но
                // строкой раньше он уже зашагал к Фалькаану за 185 и не обернулся.
                //
                // ОБМЕНА «ДАЛЬНИЙ НА ДАЛЬНИЙ» ЗДЕСЬ БЫТЬ НЕ МОЖЕТ, и это не осторожность,
                // а свойство отбора по карте: он пропускает всё ближе Cfg().QuestGiverRange
                // (`d < Cfg().QuestGiverRange -> continue`), потому что близкое — работа
                // обзора. Значит любой найденный обзором заведомо ближе того, к кому мы
                // идём, и пересмотр всегда сокращает дорогу.
                //
                // Спрашиваем не каждый такт: тот же срок, что и обычный опрос
                // квестодателя. Отбор в обзоре опрашивает ядро по каждому существу
                // вокруг, и это не та цена, которую платят шестьдесят раз в секунду.
                //
                // И НЕ ОГЛЯДЫВАЕМСЯ НА ХВОСТЕ (Кодекс). Утверждение выше — «найденный
                // обзором заведомо ближе» — верно ровно до тех пор, пока до старой цели
                // дальше радиуса обзора. Подойдя ближе, спутник мог бы сменить цель на
                // равную или дальнюю, и обещание перестало бы быть правдой. Условие ниже
                // делает его точным, а заодно снимает опрос там, где идти осталось всего
                // ничего.
                if (!c.SeekRecheckMs && d > Cfg().QuestGiverRange)
                {
                    c.SeekRecheckMs = Cfg().QuestIntervalMs;
                    if (Creature* near = NearestQuestGiver(c, self))
                    {
                        c.GiverGuid = near->GetGUID();
                        c.GiverMs = 0;
                        c.GiverDist = self->GetExactDist(near);
                        c.GiverRange = Cfg().QuestGiverRange;
                        c.TalkDiagDone = false;
                        TC_LOG_INFO("server.worldserver",
                            "Constellation ПОХОД {}: по дороге к {} ({} ярдов) нашёлся {} ({}) "
                            "в {:.0f} ярдах — бросаю дальнего",
                            self->GetName(), c.SeekEntry, uint32(d), near->GetName(),
                            near->GetEntry(), c.GiverDist);
                        // ДАЛЬНЮЮ ТОЧКУ НЕ ОТМЕЧАЕМ ПРОЙДЕННОЙ: мы до неё не дошли, и
                        // вернуться к ней потом — законно. Отсрочку ставит только приход
                        // и только неудача.
                        Switch(c, self, Behavior::Idle, "рядом нашёлся квестодатель");
                        return;
                    }
                }
            ...
            case Behavior::Travelling:
            {
                ...
                if (c.Move.Stalled || c.WalkStuckMs > 30000 || c.ModeMs > Cfg().WalkCapMs)
                {
                    ...
                    // минуты выбирается тот же квест, та же цель и тот же отказ. Замер: две
                    // тауренки по восемь заходов за окно к цели на 150 ярдов ниже себя — вниз
                    // ведёт подъёмник, а его в сетке путей нет. Срок тот же, что у соседней
                    // ветки «пришёл — целей нет»: десять минут.
                    if (c.TravelQuest)
                        c.TravelBackoff[c.TravelQuest] = 600000;
            ...
            case Behavior::ApproachingTarget:
            {
                ...
                if (c.EngageRange == 0.0f)
                {
                    ...
                    if (Cfg().Abilities)
                        if (uint32 sp = PickAttackSpell(self, target, 0, true))
            ...
            case Behavior::Attacking:
            {
                ...
                if (!victim || !victim->IsAlive())
                {
                    ...
                    if (won)
                    {
                        ...
                        // ПОЗИЦИЯ ПОБЕДИТЕЛЯ, а не убитого, и это приближение: для дальнего
                        // боя клетки могут разойтись. Взять позицию цели здесь можно было бы
                        // только сохранив её заранее — ядро уже очистило жертву, потому мы и в
                        // этой ветке. Кодекс (задача 113) назвал это допустимым при сложении по
                        // 3x3, но не «невозможным» — так оно и записано.
                        auto& ks = c.KillSpots[SpotKey(self->GetMapId(),
                                                       self->GetPositionX(), self->GetPositionY())];
                        if (ks < 60000)
                            ++ks;
                    }
                    else if (victim)
    ...
    static bool IsOverTime(SpellInfo const* si)
    {
        return si && (si->HasAura(SPELL_AURA_PERIODIC_DAMAGE)
                   || si->HasAura(SPELL_AURA_PERIODIC_DAMAGE_PERCENT)
                   || si->HasAura(SPELL_AURA_PERIODIC_LEECH));
    }
                          ...
                          bool overTimeCounts = true)
    {
        ...
        // УРОН ВО ВРЕМЕНИ — ТОЖЕ УРОН. НЕБОЛЬШАЯ ДЫРА, И ИМЕННО НЕБОЛЬШАЯ.
        //
        // Шесть эффектов выше — все ПРЯМЫЕ, поэтому заклинание, весь урон которого сидит в
        // периодической ауре, отвечало на вопрос «бьёт ли это» отрицательно и не могло
        // быть выбрано никогда — не выбиралось хуже, а не было видно вовсе.
        //
        // РАЗМЕР ДЫРЫ ИЗМЕРЕН, А НЕ ОЦЕНЁН НА ГЛАЗ, и первая догадка была неверна. Палитра
        // печатает каждую запись книги с пометкой «бьёт/лечит/никак» и формой эффектов,
        // так что вопрос решается по боевому журналу: на пяти спутниках из 374 записей
        // «никак» периодический урон несёт РОВНО ОДНА — монашья 117952, у которой три
        // эффекта ауры и ни одного прямого. Shadow Word: Pain в это число НЕ входит: у
        // него рядом с аурой стоит эффект 2, школьный урон, и старая сборка честно
        // помечала его «бьёт [годится]». Жрец звучал одним Mind Blast не потому, что
        // ядро прятало Pain, а потому что выбор был максимумом по уровню заклинания.
        //
        // Так что это правка настоящей, но узкой дыры, и объяснением она ничему не
        // служит. Спрашиваем ядро тем же способом, каким оно спрашивает себя:
        // SpellInfo::HasAura обходит эффекты и проверяет тип ауры.

        if (!si)
            return false;
        // lazy: РАСШИРЕНА ТОЛЬКО БЬЮЩАЯ СТОРОНА, и это осознанная асимметрия. Симметричное
        // «лечение во времени» впустило бы постепенное лечение в PickSelfHeal, который
        // зовут при 35 % здоровья, — а у того отбора есть своё записанное решение с
        // условием пересмотра: сперва счётчики вылеченного и перелеченного, иначе нечем
        // будет доказать, что стало лучше. Условие не выполнено, решение в силе. Предел:
        // спутник, у которого из лечения есть только постепенное (друид с Rejuvenation),
        // по-прежнему не лечится. Путь наверх — те самые счётчики.
        if (wantHeal)
        {
        ...
        for (SpellEffectInfo const& eff : si->GetEffects())
        {
            ...
            if (!trigger || !eff.TriggerSpell)
                continue;
            if (SpellDoes(sSpellMgr->GetSpellInfo(eff.TriggerSpell, diff), wantHeal, diff,
                          depth - 1, overTimeCounts))
                return true;
        }
        return false;
    }
    ...
    // вместе с персонажем.
    // ЛЕСТНИЦА ПРИОРИТЕТА, А НЕ ОДИН МАКСИМУМ.
    //
    // Прежний выбор возвращал единственное заклинание с наибольшим уровнем, и то, что
    // спутник знает ровно одно умение, было не дефектом реализации, а арифметикой:
    // максимум множества — это один элемент. Замер на боевом подтвердил снаружи: у трёх
    // спутников из четырёх за все их бои ровно ОДНО применённое заклинание, у четвёртого
    // два (платное и бесплатное). Перепись жреца 7 уровня: в книге Smite, Shadow Word:
    // Pain, Mind Blast; звучал только Mind Blast, потому что у него уровень выше.
    //
    // Ступени спрашивают у ЯДРА, а не у таблицы по классам, поэтому правило одно на все
    // классы и на все уровни:
    //   0 ПОДДЕРЖАНИЕ — заклинание вешает периодический урон, и МОЕЙ такой ауры на цели
    //                   сейчас нет (Unit::HasAura с нашим GUID);
    //   1 ПО ГОТОВНОСТИ — ядро объявило откат (SpellInfo::GetRecoveryTime), и он уже прошёл.
    //                   Ступень значит БУКВАЛЬНО это и ничего больше: Кодекс верно замечает,
    //                   что обычные роторные генераторы вроде Crusader Strike тоже несут свой
    //                   откат и попадут сюда. Так и надо — «нажимать по готовности» это и есть
    //                   правило для всего, у чего откат объявлен, а называть это всплеском
    //                   значило бы обещать больше, чем проверено;
    //   2 ЗАПОЛНИТЕЛЬ — всё остальное;
    //   3 В ПОСЛЕДНЮЮ ОЧЕРЕДЬ — периодическое, которое сейчас применять незачем: моя аура уже
    //                   висит, либо предыдущее такое же не легло. Берём, ТОЛЬКО если больше
    //                   нечего: перевешивать висящий периодический урон каждый такт — потеря,
    //                   а не ротация, но у спутника, у которого из урона одно оно, отнимать
    //                   его нельзя.
    // Внутри ступени порядок прежний: платное впереди бесплатного, уровень заклинания как
    // суррогат силы.
    static constexpr int RUNG_KEEP = 0, RUNG_READY = 1, RUNG_FILL = 2, RUNG_LAST = 3, RUNGS = 4;

    // flatRanking — выдать ПРЕЖНИЙ единственный максимум, без ступеней. Нужен ровно одному
    // месту: расчёту дистанции боя. Кодекс: та дистанция считается один раз за бой и
    // кэшируется, а в начале боя аур ещё нет, поэтому ступень поддержания там выигрывает
                           ...
                           bool flatRanking = false) const
    {
        ...
        // ПРЕДЫДУЩЕЕ ПЕРИОДИЧЕСКОЕ НЕ ЛЕГЛО — В ЭТОТ РАЗ ПЕРИОДИЧЕСКОЕ НЕ ПРЕДЛАГАЕМ ВОВСЕ.
        // Без этого два не ложащихся периодических заклинания чередовались бы на ступени
        // поддержания бесконечно, ни разу не дойдя до заполнителя (Кодекс). Так же, как и
        // с иммунитетом: правильный вопрос — Unit::IsImmunedToSpell, но все его вызовы в
        // ядре передают маску эффектов, которую оно считает при выборе целей и которой на
        ...
        // порядок по уровню заклинания, и это ПРИЗНАННЫЙ СУРРОГАТ — уровень не измеряет
        // силу (Кодекс). Настоящая мера это урон за произнесение, и её надо намерить, а не
        // придумать; до тех пор суррогат честнее случайного выбора.
        uint32 bestPaid[RUNGS] = { 0, 0, 0, 0 }, bestPaidLevel[RUNGS] = { 0, 0, 0, 0 };
        uint32 bestFreeId[RUNGS] = { 0, 0, 0, 0 }, bestFreeLevel[RUNGS] = { 0, 0, 0, 0 };
        for (auto const& [id, ps] : self->GetSpellMap())
        {
            ...
            // РАЗВЕДКА ДАЛЬНОСТИ СМОТРИТ ПРЕЖНИМИ ГЛАЗАМИ, а не только прежним порядком.
            // Кодекс: flatRanking воспроизводил ранжирование, но состав кандидатов уже
            // другой — расширенный предикат пускает сюда чисто периодические заклинания,
            // а дистанция считается один раз за бой и кэшируется. Значит и допуск там
            // должен быть прежним.
            ...
            if (uint32 aura = OverTimeAuraId(si, diff))
            {
                ...
                // выиграть — то есть перевешивалось бы каждый такт, ровно ломая ту ротацию,
            ...
            if (free)
            {
                if (!bestFreeId[rung] || si->SpellLevel > bestFreeLevel[rung])
                    { bestFreeId[rung] = id; bestFreeLevel[rung] = si->SpellLevel; }
            }
            else
            {
                if (!bestPaid[rung] || si->SpellLevel > bestPaidLevel[rung])
                    { bestPaid[rung] = id; bestPaidLevel[rung] = si->SpellLevel; }
            }
        }
        ...
        // Ступень при этом СИЛЬНЕЕ платности: бесплатный периодический урон стоит дороже
        // платного заполнителя, потому что держится на цели сам.
    ...
    void GatherLeave(Companion& c, Player* self, uint32 backoffMs, bool fruitless, char const* why)
    {
        if (c.GatherSpawnId)
        {
            ...
            if (fruitless)
                // ХОЛОСТОЙ ЗАХОД СЧИТАЕТСЯ ТОЧКЕ, И ТОЛЬКО ТОЧКЕ. Засечка виду отсюда снята
                // вместе со всей отсечкой по виду (круг 66): она закрывала работающую
        ...
        // спутник приписал бы двери изменение квестов, случившееся совсем в другом месте.
        if (c.LockCastSpawn)
        {
            // УХОДИМ — ЗНАЧИТ ГАСИМ И СВОЁ ЗАКЛИНАНИЕ (Кодекс, задача 143).
            //
            // Отмена стояла только на ветке тайм-аута, но резерв освобождается и здесь, а сюда
            // приходят из боя, из смерти и из смены цели — то есть с ЖИВЫМ кастом. Тогда
            // отпущенную точку мог занять напарник поверх чужого чтения, и обещание «касты не
            // пересекаются» снова держалось на рассуждении о том, что делает диспетчер
            // поведения, вместо местной гарантии.
            //
            // Теперь она местная: живой спутник не может оставить за собой резерв с идущим
    ...
    // строкой без объяснений вовсе. Копия правила, у которой половина обоснования видна только
    // одному читателю, — это копия, которая разойдётся.
    //
    // ЧТО ОНО НЕ РЕШАЕТ, И ЭТО ВАЖНО. Оно говорит только «мир такое разрешает». Оно НЕ выбирает
    // между несколькими годными — этим занимается цвет и ключ «одноразовые прежде повторяемых»,
    // и он у каждого вызывающего свой, потому что вопросы разные: у меню это «что взять», у
    // указателя карты — «куда идти». И оно НЕ помнит, что уже пробовали: память об отказах
    // принадлежит механизму, а не миру.
    //
    // СЕЗОННОЕ — не наша работа (оператор, 2026-09-03: «это категория сезонных квестов, нас
    // такое сейчас не интересует»). Признак — сортировка квеста; таких в базе 422, и вне своего
    // события они висят в журнале мёртвым грузом, как 55660 у всех 122.
    //
    ...
    // ГУУБЕР С МНОГОКРАТНЫМ ВЗАИМОДЕЙСТВИЕМ: применение к нему наблюдаемого следа не
    // оставляет. Вынесено в предикат, потому что решение о пути принимается до отправки,
    // а прежде флаг читался уже после неё.
    static bool multiInteractGoober(GameObject const* go)
    {
        GameObjectTemplate const* gt = go ? go->GetGOInfo() : nullptr;
        return gt && gt->type == GAMEOBJECT_TYPE_GOOBER && gt->goober.AllowMultiInteract != 0;
    }
    ...
    bool SpawnDestination(Player* self, uint32 entry, Position* out) const
    {
        ...
        {
            float d = self->GetExactDist2d(pos.GetPositionX(), pos.GetPositionY());
            if (d < bestDist && !ReachableTier(self, pos.GetPositionX(), pos.GetPositionY(),
                                              pos.GetPositionZ()))
                continue;                       // чужой ярус
            if (d < bestDist)
                { bestDist = d; best = &pos; }
    ...
    bool DeathSpotBlocked(Companion const& c, Player const* self, uint32 mapId, float x, float y,
                          ...
                          uint32* levelOut = nullptr,
                          uint32* killsOut = nullptr, float* nearestOut = nullptr) const
    {
        uint32 total = 0, worstCount = 0, level = 0, kills = 0;
        uint64 worst = 0;
        float nearest = 100000.0f;
        // Центр клетки восстанавливается по той же разбивке, что и в SpotKey: floor(x/50).
        int32 const bx = int32(std::floor(x / 50.0f));
        int32 const by = int32(std::floor(y / 50.0f));
        for (int dx = -1; dx <= 1; ++dx)
            for (int dy = -1; dy <= 1; ++dy)
            {
                uint64 const key = SpotKey(mapId, x + dx * 50.0f, y + dy * 50.0f);
                // ПОБЕДЫ СЧИТАЕМ ПО ВСЕМ ДЕВЯТИ, а не только там, где были смерти: место
                // проходимо, если я побеждал ГДЕ-ТО в этой окрестности. И только если их
                // спрашивают: маршрутные вызовы за этот поиск платить не должны (Кодекс, 112).
                if (killsOut)
                {
                    auto k = c.KillSpots.find(key);
                    if (k != c.KillSpots.end())
                        kills += k->second;
                }
                ...
                // РАССТОЯНИЕ ДО КЛЕТКИ, ГДЕ СМЕРТЬ ДЕЙСТВИТЕЛЬНО БЫЛА. Сумма по 3x3 отвечает
                // на вопрос «опасно ли это скопление», а не «опасна ли ЭТА точка»: ореол
                // тянется на 106 ярдов от крайней смертельной клетки (Кодекс, задача 111).
                //
                // МЕРИМ ДО ГРАНИЦЫ КЛЕТКИ, А НЕ ДО ЕЁ ЦЕНТРА. Первая версия брала центр, и
                // Кодекс (задача 112) показал, чем это плохо: настоящее место смерти внутри
                // клетки, поэтому центр ошибается на величину до sqrt(25^2+25^2) = 35 ярдов,
                // и цель в шестидесяти ярдах от трупа могла оказаться в девяноста пяти от
                // центра и пройти фильтр — ровно измеренный случай. Расстояние до
                // прямоугольника НИКОГДА не больше расстояния до самой смерти, поэтому порог
                // не может пропустить того, кто ближе порога к настоящему трупу.
                float const cx0 = float(bx + dx) * 50.0f;
                float const cy0 = float(by + dy) * 50.0f;
                float const offX = std::max(0.0f, std::max(cx0 - x, x - (cx0 + 50.0f)));
                float const offY = std::max(0.0f, std::max(cy0 - y, y - (cy0 + 50.0f)));
                float const dd = std::sqrt(offX * offX + offY * offY);
                if (dd < nearest)
                    nearest = dd;
            }
    ...
    // ---------------------------------------------------------------- ЯРУС ЦЕЛИ
    // ПЛОСКАЯ МЕРКА ВЫБИРАЕТ ЦЕЛИ НА ЧУЖОМ ЭТАЖЕ, И ЭТО НЕ ТЕОРИЯ.
    //
    // Замер на боевом 2026-09-06. Тринадцать рыцарей смерти созданы на палубе Акеруса,
    // z = 426. Первой же строкой жизни модуль выбрал Salanar the Horseman и написал
    // «иду к ближайшему узлу в 29 ярдах». Все четыре узла его маршрута 10356800 лежат
    // на z = 154 — 272 ярда ОТВЕСНО ВНИЗ. Шаг хождения берёт высоту линейной долей до
    // цели (см. «ВЫСОТА БЕРЁТСЯ ПО ДОЛЕ ПРОЙДЕННОГО ОТРЕЗКА»), поэтому за 29 ярдов
    // горизонтали он ровно опустил их СКВОЗЬ палубу. Ни падения, ни урона, ни смерти:
    // ядро не видело падения, оно видело обычную ходьбу — в журнале ноль строк про
    // падение и ноль гибелей. Итог: 13 ДК и 2 ДХ имеют НОЛЬ сданных квестов, тогда как
    // воины 111, разбойники 95, монахи 81, маги 80.
    //
    // ДЕШЁВЫЙ ПОРОГ БЕРЁМ У ЯДРА. PathGenerator::GetPolyByLocation (PathGenerator.cpp:141)
    // ищет полигон точки в коробке {3, 5, 3}; не нашёл — расширяет ТОЛЬКО по высоте:
    // extents[1] = 50.0f (там же, :152). Дальше пятидесяти ярдов по вертикали ядро
    // отвечает INVALID_POLYREF. Значит 50 это его мерка «та же поверхность», не моя.
    //
    // ДАЛЬШЕ ПЯТИДЕСЯТИ — НЕ ОТКАЗ, А ВОПРОС ПОСТРОИТЕЛЮ. Первая редакция этой правки
    // спрашивала вместо него Map::GetHeight от своей высоты, и Кодекс её отверг по делу:
    // такой запрос смотрит ВНИЗ. Он не видит ни цель выше по склону, ни палубу над собой —
    // а тринадцать ДК сейчас стоят как раз ПОД палубой, и по такому пробнику им нечего
    // было бы выбрать вовсе. Спрашиваем то, что и решает дело: построит ли ядро маршрут.
    // Способ ровно тот же, каким модуль уже проверяет точку подхода (см. ниже, «ОТВЕРГНУТУЮ
    // ТОЧКУ НЕ ПРЕДЛАГАТЬ СНОВА»).
    //
    // ЦЕНА. Построение пути идёт ТОЛЬКО для кандидата, который и по высоте подозрителен,
    // и НЕ ХУЖЕ нынешнего лучшего — то есть по нескольку раз за выбор, а на ровной
    // местности не идёт ни разу. «Не хуже», а не «лучше» (Кодекс, третий проход): при
    // равенстве в пределах 0.01 ярда проверка пройдёт и у того, кто затем проиграет отбор
    // по меньшему номеру точки. Это лишняя проверка пути, а не лишний выбор.
    //
    // КЛАСС ЗАДАЧИ, а не случай: «взять ближайшую цель из таблицы мировых точек по
    // плоской мерке». Здесь он закрыт в четырёх местах — квестодатель, ближайший спавн
    // вида, торговец-ремонтник, точка сбора. Пятое место, узел маршрута ходячего NPC,
    // обходится дешёвой проверкой без построителя: узел лишь уточняет уже принятую цель.
    ...
    bool ReachableTier(Player const* self, float x, float y, float z) const
    {
        ...
        // а граница задачи: здесь решается ЯРУС, а не достижимость вообще. Достижимость
        // спрашивается ниже по течению — построитель, которым спутник идёт, отвечает
        // «до квестодателя по карте не дойти», и это работает. Делать её точной здесь
        // значило бы строить путь КАЖДОМУ кандидату на каждом выборе — та самая цена,
        // которая уже упирала мир в ядро.
        if (std::fabs(z - self->GetPositionZ()) <= TIER_Z)
            return true;                    // ядро само считает это одной поверхностью
        PathGenerator path(self);
        if (!path.CalculatePath(x, y, z, false))
            return false;
        // МАСКА — ПО ТОМУ, ЧТО ЯДРО САМО СЧИТАЕТ НАСТОЯЩИМ МАРШРУТОМ (Кодекс, второй проход).
        // Здесь стояло (NOPATH | SHORTCUT | INCOMPLETE), и этого мало. Когда сетки нет,
        // плитка начала или конца не загружена, либо существо летает, CalculatePath строит
        // ПРЯМУЮ и помечает её PATHFIND_NORMAL | PATHFIND_NOT_USING_PATH, возвращая true
        // (PathGenerator.cpp:77-83). Такой ответ прошёл бы прежнюю проверку и разрешил бы
        // ровно то движение сквозь геометрию, ради которого проверка и написана.
        // FARFROMPOLY ядро ставит, когда конец пути дальше семи ярдов от ближайшего
        // полигона (там же, :216 — «distToEndPoly > 7.0f»), то есть «эта точка не лежит
        // на проходимой поверхности». Обе метки в отказ.
        return !(path.GetPathType() & (PATHFIND_NOPATH | PATHFIND_SHORTCUT | PATHFIND_INCOMPLETE
                                     | PATHFIND_NOT_USING_PATH | PATHFIND_FARFROMPOLY));
    }

    // ---------------------------------------------------------------- ПЕРЕПИСЬ КНИГИ У ЗАЧЁТА
    // ЗАЧЕМ ЭТА СТРОКА СУЩЕСТВУЕТ, ЕСЛИ ОНА НИЧЕГО НЕ ДЕЛАЕТ.
    //
    // Замер на боевом 2026-09-06: двенадцать рыцарей смерти сдали 12593 и 12619, взяли
    // 12842 «Runeforging: Preparation For Battle» и встали. Цель квеста — `Type 0`,
    // существо 28357, количество 1: это ЗАЧЁТ, а не предмет и не убийство. Ядро выдаёт
    // его в C++: spell_chapter1_runeforging_credit (chapter1.cpp:1147-1165) висит на
    // EFFECT_1 восьми рунических заклинаний и при незакрытом 12842 кастует 54586.
    //
    // Связь «руна -> зачёт» живёт только в spell_script_names и в самом скрипте, поэтому
    // указатель _focusCreditSpells найти её не может: он строится по эффектам KILL_CREDIT
    // САМОГО заклинания (:7373-7382), а у руны такого эффекта нет.
    //
    // Первый заход был шире: «есть незакрытая цель-зачёт — иди к любому подходящему фокусу
    // и произнеси оттуда что-нибудь из книги». Кодекс его отверг, и по делу: наличие
    // незакрытого зачёта НЕ доказывает, что произвольное заклинание с фокусом его
    // продвинет, а приговор по сдвигу целей записал бы в успех любое постороннее движение
    // за те же пятнадцать секунд. Поэтому здесь не осталось ни одного каста.
    //
    // Осталось ровно то, чего не хватает для следующего шага: ЧИСЛО. Книга спутника не
    // хранится в базе (characters_1127.character_spell — четырнадцать строк на весь мир),
    // поэтому «знают ли они руну» нельзя ни прочитать, ни угадать. Пусть скажет сам мир.
    // Печатается ОДИН раз на спутника, и только когда у него есть незакрытая
    // цель-зачёт; пока её нет, проверка повторяется не чаще раза в минуту.
    void BookDiag(Companion& c, Player* self) const
    {
        // СТОРОЖ ДВОЙНОЙ, И ЭТО НЕ ПЕРЕСТРАХОВКА (Кодекс, второй проход).
        // Флага мало: пока зачётного квеста нет, ранний выход не ставил его вовсе, и
        // журнал квестов пересматривался на КАЖДОМ отборе точек — то есть по сути
        // каждый такт. Сделать перепись терминальной с первого зова тоже нельзя:
        // спутник возьмёт зачётный квест позже, и измерение не состоится вовсе.
        // Поэтому флаг закрывает «уже напечатали», а таймер — «пока не за что».
        if (c.BookDiagDone || c.BookDiagMs)
            return;
        uint32 openCredit = 0;
        for (auto const& [qid, st] : self->getQuestStatusMap())
        {
            ...
            for (QuestObjective const& obj : q->GetObjectives())
                if (obj.Type == QUEST_OBJECTIVE_MONSTER && obj.ObjectID > 0
                    ...
                    { openCredit = qid; break; }
            if (openCredit)
                break;
        }
        if (!openCredit)
            { c.BookDiagMs = 60000; return; }        // не за что цепляться — вернёмся через минуту

        // «ЗНАЕТ» И «ЛЕЖИТ В КНИГЕ» — РАЗНЫЕ ВЕЩИ, И РУНЫ ЖИВУТ РОВНО В ЭТОМ ЗАЗОРЕ.
        //
        // Оператор, 2026-09-06: «ДК руны — в особом окне». Ядро говорит то же самое:
        // HasSpell и HasActiveSpell отличаются ровно одним полем (Player.cpp:3735-3753) —
        //     HasSpell:       state != PLAYERSPELL_REMOVED && !disabled
        //     HasActiveSpell: state != PLAYERSPELL_REMOVED && itr->second.active && !disabled
        // `active` значит «показывать в книге заклинаний». Руна персонажу известна, но
        // показывается не в книге, а в окне рунной кузни, поэтому HasActiveSpell по ней
        // ЛОЖЬ. Считай я только книгу — получил бы ноль и записал бы «руны не изучены»,
        // то есть придумал бы третью подряд ложную причину. Считаем оба числа.
        //
        // Это же различие — ответ на то, почему выбор боевого умения ниже спрашивает
        // именно HasActiveSpell и прав: там нужна книга клиента. Здесь нужна не книга.
        uint32 known = 0, inBook = 0, onMap = 0, firstId = 0, firstFocus = 0;
        auto fm = _focusSpawns.find(self->GetMapId());
        for (auto const& [id, ps] : self->GetSpellMap())
        {
            if (!self->HasSpell(id))
                continue;                       // снято или выключено — не считаем
            SpellInfo const* si = sSpellMgr->GetSpellInfo(id, self->GetMap()->GetDifficultyID());
            ...
            if (self->HasActiveSpell(id))
                ++inBook;
            if (!firstId)
                { firstId = id; firstFocus = si->RequiresSpellFocus; }
            if (fm != _focusSpawns.end() && fm->second.count(si->RequiresSpellFocus))
                ++onMap;
        }
        c.BookDiagDone = true;
        TC_LOG_INFO("server.worldserver",
            "Constellation КНИГА {}: незакрытый зачёт по квесту {}; заклинаний с фокусом известно {}, "
            "из них в книге {}, с фокусом на этой карте {}, первое {} (фокус {})",
            self->GetName(), openCredit, known, inBook, onMap, firstId, firstFocus);
    }
                           ...
                           uint32 entry) const
    {
        for (uint32 qid : sObjectMgr->GetCreatureQuestRelations(entry))
        {
            ...
            if (!q || self->GetQuestStatus(qid) != QUEST_STATUS_NONE)
                continue;
            // ТО ЖЕ УСЛОВИЕ, ЧТО И У МЕНЮ, И ТЕПЕРЬ БУКВАЛЬНО ТО ЖЕ. Объяснения — при самом
            // правиле; здесь их дублировать незачем, а не иметь их вовсе было хуже.
            if (!PassesWorldRules(self, q))
                continue;
            // НЕИЗВЕСТНЫЙ УРОВЕНЬ — НЕ ПОВОД ОТКАЗАТЬСЯ. ЭТО СТОИЛО ЦЕЛОЙ СТАРТОВОЙ ЗОНЫ.
            //
            // Здесь стояло `>= 3`, то есть отказ и красному, и неизвестному. Замер на
            // боевом 2026-09-06: восемь людей уходят из Долины Северных Отрогов после
            // ОДНОГО вступительного квеста, и журнал говорит «рядом существ 5, нечего
            // предложить 5» — при том что у одного Marshal McBride тридцать квестов,
            // у Sergeant Willem восемнадцать, у Brother Paxton девять.
            //
            // Причина в одном числе. У всех квестов Монастыря ContentTuningID = 1476,
            // у квестов Златолесья, которые спутники брали, — 73. А Player::GetQuestLevel
            // (Player.cpp:14231) возвращает НОЛЬ ровно в одном случае — когда
            // sDB2Manager.GetContentTuningData для этой настройки ничего не дала:
            //     if (Optional<ContentTuningLevels> questLevels = sDB2Manager.GetContentTuningData(...))
            //     { ... return std::min(level, maxLevel); }
            //     return 0;
            // Ноль превращался в цвет 4 «неизвестно» и в отказ — так модуль отбрасывал
            // всю цепочку Blackrock Invasion и уходил в Златолесье, где настройка есть.
            //
            // ПОЧЕМУ БРАТЬ ТЕПЕРЬ БЕЗОПАСНО. Ворота ядра стоят строкой выше: CanTakeQuest
            // уже проверил минимальный уровень, расу, класс, предыдущий квест и
            // исключающую группу — ровно то, что решает, увидит ли квест живой игрок.
            // Неизвестный уровень не значит «слишком трудно», он значит «у ядра нет
            // числа». Отказывать по отсутствию числа — быть строже клиента, а инвариант
            // модуля обратный. Кодекс, проход по этой правке: обоснованно; худший исход —
            // взятый неподходящий квест, который снимут отсрочка по гибелям и предел
            // дороги, а не потеря состава.
            //
            // Красное (цвет 3) отвергается по-прежнему: там уровень ИЗВЕСТЕН и выше
            // нашего больше, чем разрешает QuestMaxAbove.
            if (QuestColour(self, q) == 3)
                continue;                       // красное — не по нам
            return qid;
        }
        return 0;
    }
                        ...
                        Position* pos, uint32* questOut) const
    {
        ...
        for (Giver const& g : it->second)
        {
            ...
            if (!qid)
                continue;
            if (!ReachableTier(self, g.Where.GetPositionX(), g.Where.GetPositionY(),
                               g.Where.GetPositionZ()))
        ...
        *questOut = bestQuest;
        ...
        if (best->PathId)
            if (WaypointPath const* path = sWaypointMgr->GetPath(best->PathId))
            {
                ...
                {
                    if (std::fabs(node.Z - self->GetPositionZ()) > TIER_Z)
                        continue;   // узел под палубой — ровно по нему ДК и ушли вниз
            ...
            if (mine)
                if (FactionTemplateEntry const* theirs = sFactionTemplateStore.LookupEntry(m.Faction))
                    if (mine->IsHostileTo(theirs))
                        continue;
            float const d = self->GetExactDist2d(m.Where.GetPositionX(), m.Where.GetPositionY());
            if (d < bestD && !ReachableTier(self, m.Where.GetPositionX(), m.Where.GetPositionY(),
                                            m.Where.GetPositionZ()))
                continue;                       // чужой ярус
            if (d < bestD)
                { bestD = d; best = &m; }
        }
    ...
    {
        uint32       QuestId      = 0;
        // ИМЯ НЕ `Quest`: поле с именем типа меняет смысл `Quest` внутри структуры, и gcc это
        // отвергает (-Wchanges-meaning). `Template` вернее и по сути — это шаблон, а не квест.
        Quest const* Template     = nullptr;
        uint8        Colour       = 4;
        uint32       Offered      = 0;      // сегодня никем не читается — см. комментарий у вызова
        uint32       SkippedRed   = 0;
        uint32       UnknownLevel = 0;
    };
    ...
    MenuPick PickFromQuestMenu(Player const* self, QuestRefusedFn refused, void const* user) const
    {
        MenuPick pick;
        uint8 bestRank = 255;
        // БЕЗ ПРОВЕРКИ НА null, КАК И БЫЛО. Здесь стояла защита `!self || !self->PlayerTalkClass`,
        // которой в исходном цикле не было, — и она ломала единственное, ради чего это извлечение
        // выделено в отдельный коммит: доказуемое отсутствие изменений (Кодекс это и назвал).
        // Защищать ей нечего: строка ниже на месте вызова тоже разыменовывает `PlayerTalkClass`
        // без проверки, так что при null падение случилось бы раньше. Проверка появится вместе с
        // движковым вызывающим, которому она действительно нужна.
        QuestMenu const& menu = self->PlayerTalkClass->GetQuestMenu();
        for (uint8 i = 0; i < menu.GetMenuItemCount(); ++i)
        {
            uint32 const qid = menu.GetItem(i).QuestId;
            ...
            // МИРОВЫЕ ПРАВИЛА — ОДНИМ ВОПРОСОМ. Ворота ядра, сезонное и чужая профессия
            // проверяются в `PassesWorldRules`, где лежат и объяснения; здесь остаётся то, что
            // относится к МЕНЮ, — порядок, цвет и ключ «одноразовые прежде повторяемых».
            Quest const* q = sObjectMgr->GetQuestTemplate(qid);
            if (!PassesWorldRules(self, q))
                continue;
            ++pick.Offered;
            uint8 const colour = QuestColour(self, q);
            // НЕИЗВЕСТНЫЙ УРОВЕНЬ БОЛЬШЕ НЕ ПРОПУСКАЕТСЯ, И ЭТО ВТОРАЯ ПОЛОВИНА КРУГА 70.
            //
            // Там отказ по неизвестному сняли в ОТБОРЕ квестодателя, а здесь, в меню,
            // он остался — и правка вышла инертной: дойди спутник до Макбрайда, он всё
            // равно не взял бы ничего. Замер это и показал: за всю жизнь состава у
            // Marshal McBride (197) взято НОЛЬ квестов.
            //
            // Ноль у GetQuestLevel означает «у ядра нет данных настройки контента»
            // (Player.cpp:14231), а не «слишком трудно». Ворота ядра — CanTakeQuest —
            // стоят выше в этом же цикле и уже сказали «можно». Красное по-прежнему
            // не берём: там уровень ИЗВЕСТЕН и выше нашего.
            //
            // Порядок сохраняется сам: ранг считается от цвета, у неизвестного он 4,
            // то есть хуже жёлтого, зелёного и серого. Известное предпочитается, а
            // неизвестное берётся, когда другого нет.
            if (colour == 4)
                ++pick.UnknownLevel;            // считаем, но больше НЕ пропускаем
            if (colour == 3)
                { ++pick.SkippedRed; continue; }     // красное — не по нам
            uint8 const rank = uint8((q->IsRepeatable() || q->IsDailyOrWeekly() ? 10 : 0) + colour);
            if (rank < bestRank)
                { bestRank = rank; pick.Colour = colour; pick.QuestId = qid; pick.Template = q; }
        }
        return pick;
    }
    ...
    // СКОЛЬКО РАЗ ПРИХОДИТЬ К ВЕЧНО ЗАНЯТОЙ ТОЧКЕ. Имя было ..._LOG, пока предел жил в
    // строке журнала; теперь он задаёт отбор, и имя это говорит (Кодекс, задача 130).
    static uint8 const GOOBER_BUSY_TRIES = 3;
    ...
    bool FindGatherCandidate(Companion& c, Player* self) const
    {
        ...
        static uint8 const GOOBER_TRIES = GOOBER_BUSY_TRIES;
        uint32 goobers = 0;
                    ...
                    if (self->GetQuestStatus(qid) == QUEST_STATUS_INCOMPLETE)
                        if (auto gq = gm->second.find(qid); gq != gm->second.end())
                            for (GatherSpawn const& sp : gq->second)
                            {
                                ...
                                // ТОЧКА, КОТОРАЯ ВЕЧНО ЗАНЯТА, — тоже тупик, но тупик ТОЧКИ.
                                // Считаются только заходы, при которых кликнуть не удалось:
                                // готовый клик тратит точку сразу и уводит её в GooberDone.
                                if (auto idle = c.GooberIdle.find(sp.SpawnId);
                                    idle != c.GooberIdle.end() && idle->second >= GOOBER_TRIES)
                                    continue;       // трижды пришли, и трижды объект был занят
                                pool.push_back(&sp);
                                ++goobers;
                            }
        ...
        BookDiag(c, self);              // одна строка на спутника, ничего не делает
        ...
        for (GatherSpawn const* pc : pool)
        {
            ...
            if (auto empt = c.GatherEmpty.find(sp.SpawnId); empt != c.GatherEmpty.end() && empt->second >= 8)
                continue;
            // КВЕСТОВАЯ ТОЧКА ОДНОРАЗОВА, И ЭТО ЕДИНСТВЕННОЕ, ЧТО ЗДЕСЬ НАДО ПРОВЕРИТЬ.
            // Отработанная точка лежит в GooberDone, вечно занятая — в GooberIdle; обе
            // отсечки по ТОЧКЕ. Отсечки по виду больше нет: см. объявление полей спутника.
            ...
            if (d > 200.0f)
                continue;
            if ((!found || d < best)
                && !ReachableTier(self, sp.Where.GetPositionX(), sp.Where.GetPositionY(),
                                  sp.Where.GetPositionZ()))
                continue;                       // чужой ярус
            if (!found || d < best)
            {
                best = d;
                             ...
                             Constellation::Ai::ObjectiveScan* out, Companion* diag) const
    {
        ...
        for (Creature* creature : around)
        {
            ...
            // ГДЕ МЕНЯ УЖЕ УБИВАЛИ — ТАМ Я И ЦЕЛЬ НЕ БЕРУ.
            //
            // Замер: разбойник шестого уровня погиб ШЕСТЬ раз в одном пятне пятидесяти ярдов
            // у лагеря Северной Стражи — по два раза от каждого из трёх видов (39245, 39249,
            // 39251). Запрет по виду требует двух гибелей НА КАЖДЫЙ вид, поэтому к шестой
            // смерти он был выдан лишь дважды. Память о МЕСТЕ знала об опасности уже после
            // третьей — но её спрашивал только планировщик походов, а никакого похода тут не
            // было: воскрешение ставит спутника в тридцати ярдах от собственного тела, то
            // есть у того же лагеря, и следующую цель находит обычный поиск по сетке.
            //
            // Поэтому тот же предикат спрашиваем и здесь, о МЕСТЕ ЦЕЛИ: идти-то нам туда.
            // Порог, окрестность и снятие по уровню — общие с походами, второй меры нет.
            {
                uint64 deadly = 0;
                uint32 deadlyTotal = 0, deadlyLevel = 0, deadlyKills = 0;
                float deadlyNear = 0.0f;
                // ДВА СУЖЕНИЯ, И ОБА ИЗМЕРЕНЫ, А НЕ ПРИДУМАНЫ:
                //   • цель должна стоять в семидесяти пяти ярдах от клетки, где смерть БЫЛА,
                //     иначе ореол 3x3 закрывает и то, где не гибли;
                //   • место, где я побеждаю ЧАЩЕ, чем гибну, не запрещается. Замер: у семи
                //     рыцарей в анклаве 31 гибель и ноль побед — там запрет и нужен; у Garrick
                //     12 побед при 8 гибелях — ему тот же запрет стоил бы всей работы.
                //
                // ОТНОШЕНИЕ, А НЕ НОЛЬ (Кодекс, задача 112). Первая версия отменяла запрет от
                // единственной победы в окрестности — и разбилась о собственный же случай: у
                // Mazzok одна победа при семи гибелях в том лагере, то есть правило, писанное
                // ради него, его бы и не тронуло.
                //
                // И СТРОГО БОЛЬШЕ, А НЕ «НЕ МЕНЬШЕ» (Кодекс, задача 113): при `<` ничья снимала
                ...
                if (danger.DeadlyToFightAt(creature->GetPositionX(), creature->GetPositionY()))
                {
                    if (diag && DeathSpotBlocked(*diag, self, self->GetMapId(),
                        ...
                        TC_LOG_INFO("server.worldserver",
                            ...
                            self->GetName(), creature->GetName(), creature->GetEntry(),
                            deadlyTotal, deadlyKills, deadlyNear, deadlyLevel, deadlyLevel + 3);
                    ++rejected; continue;
                }
            }
            ...
            if (!self->IsValidAttackTarget(creature))
            {
                ...
                // СПЕЛЛКЛИК — ЭТО НЕ «ЕЩЁ РАНО», А «МЕНЯ НАДО ЩЁЛКНУТЬ».
                //
                // Правило ниже верно для пленника в Дуротаре: там невосприимчивость временная,
                // её снимает тюремщик, и ждать правильно. Но у раненого пехотинца Штормграда
                // (50047) она ПОСТОЯННАЯ и значит обратное: его не бьют и с ним не говорят — по
                // нему ЩЁЛКАЮТ, и зачёт даёт заклинание клика (`npc_spellclick_spells`: 50047 -> 93072).
                //
                // Ветка `Talking` это УМЕЕТ и умела — см. её собственный разбор выше, где этот
                // пехотинец назван поимённо. До неё просто не доходило: отбор отставлял такое
                // существо раньше, чем ветка получала шанс.
                //
                // ЗАМЕР, ИЗ-ЗА КОТОРОГО ЭТО НАШЛОСЬ: восемь человек, тридцать четыре минуты,
                // НОЛЬ переходов автомата и одна строка в журнале — «пока невосприимчив к игрокам, жду».
                //
                // Различает два случая САМ ФЛАГ, и спрашиваем мы его у ядра, а не выводим из
                // повадок. Флага нет — правило ожидания остаётся в силе целиком.
                // И ТОЧНО ТАК ЖЕ — ПРЕДМЕТ ОТ КВЕСТА.
                //
                // Триггер пожара (42940) тоже невосприимчив к игрокам, но флага клика у него
                // нет и быть не должно: он закрывается ПРЕДМЕТОМ, который выдал сам квест
                // («Spray Water» по невыбираемой цели). Замер: «Extinguishing Hope» — ноль сданных
                // из восьми при ТРЁХ ВЗЯВШИХ, и в журнале у всех троих одна строка:
                // «пока невосприимчив к игрокам — не собеседник, жду». То есть взять квест они сумели,
                // а доделать им не дал этот же сторож.
                //
                // ПЕРВАЯ ПОПЫТКА ПОЧИНКИ БЫЛА ПУСТЫШКОЙ, и это стоит записать: я снял сторожа
                // с ветки «цель по контракту заклинания предмета», а она работает только при
                // `!suitable`, то есть для существ ВНЕ списка нужных. Триггер пожара как раз в
                // списке — он и есть цель задачи, — значит та ветка для него не исполнялась никогда.
                //
                // Правило Дуротара цело: у пленника нет ни флага клика, ни предмета от квеста,
                // значит оба исключения мимо, и он по-прежнему ждёт тюремщика.
                // ПОРЯДОК УСЛОВИЙ ЗДЕСЬ — НЕ СТИЛЬ, А ЦЕНА. `QuestToolFor` стоит ПОСЛЕДНЕЙ,
                // чтобы короткое замыкание не дало ей выполниться, пока не сойдутся три дешёвых
                // признака. Первая редакция считала её до них — то есть почти на каждое живое
                // существо в обходе сетки, а обход и так самое дорогое место ветки (Кодекс, п. 4).
                //
                // И ТРЕБУЕМ НЕ ПРОСТО ПРЕДМЕТ, А ПРИВЯЗКУ К КВЕСТУ (`toolQh`), как это делает
                // рабочая ветка отбора: предмет без квеста не гарантирует зачёта, и без этой второй
                // половины цель вышла бы из ожидания без уверенности, что с ней потом что-то выйдет
                // (Кодекс, п. 5).
                //
                // КУДА ОНА ПАДАЕТ, ЕСЛИ НЕ ЖДЁТ: в `else if` ниже, который требует ТОЛЬКО
                // `byMonster` — без `!suitable` и без невосприимчивости, — и ставит `TalkCandidate`.
                // Не путать со вторым местом, где `TalkCandidate` ставится в ветке отбора: там
                // действительно требуется `!suitable`, и для цели задачи она не исполняется.
    ...
    Creature* NearestQuestGiver(Companion& c, Player* self, float rangeOverride = 0.0f) const
    {
        ...
        for (Creature* creature : around)
        {
            ...
            if (c.GiverUnreachable.count(creature->GetGUID()))
                { ++blacklisted; continue; }   // уже пробовали и не вышло
            // ЗНАК ЕСТЬ — ИДЁМ И СПРАШИВАЕМ. РЕШАЕТ МЕНЮ, А НЕ МОЯ ТАБЛИЦА.
            //
            // Оператор, 2026-09-06: «мы же настраивали user experience — есть знак,
            // идём и спрашиваем что предлагает. как они что и зачем перебирают?»
            //
            // Он прав, и замер это число даёт. Модуль брал у ядра ВЕСЬ список связей
            // существа с квестами (у Marshal McBride их тридцать — по копии на каждый
            // класс), прогонял каждую через свои ворота и шёл только если сам заранее
            // одобрил хоть одну. За окно замера: «ВЫБРАН никто» 111 раз против
            // «ВЫБРАН кто-то» 2; ни одного похода ближе 25 ярдов, зато шестнадцать на
            // 150-400; у Макбрайда, стоящего в ЧЕТЫРЁХ ярдах от места рождения, взято
            // ноль квестов за всю историю состава.
            //
            // Предварительная проверка остаётся ПРЕДПОЧТЕНИЕМ: одобренный кандидат
            // выигрывает у просто помеченного. Но вето она больше не даёт — если не
            // одобрен никто, идём к ближайшему помеченному и открываем разговор, как
            ...
            if (!TakeableQuestAt(self, &RefusedByCompanion, &c, creature->GetEntry()))
            {
                ...
                //
                // Чёрный список на квестодателя ставится в разговоре — то есть ТОЛЬКО
                // если спутник до него дошёл и открыл меню. Для одобренной цели это не
                // страшно: её выбирал не знак, а моя проверка, и путь «не дойти» имеет
                // свой запрет. А запасная цель выбрана ровно потому, что проверка её НЕ
                // одобрила — если подход сорвётся (геометрия, гибель, исчезновение),
                // запрета не будет и следующий поиск возьмёт её снова. Петля.
                //
                // Поэтому в запасные берём только того, кого спутник ВИДИТ. Это и ближе
                // к тому, что делает игрок: он идёт на знак, который видит. Для
                // одобренных целей прежнее правило «не видно — не значит не дойти»
                // остаётся в силе ниже, оно оплачено случаем гнома за стенкой мастерской.
                if (!self->IsWithinLOSInMap(creature))
                    continue;
                float const md = self->GetExactDist2d(creature);
                if (md < markedDist)
                    { markedDist = md; marked = creature; }
                continue;
            }
        ...
        // ЗАПАСНОЙ ВЫБОР: помеченный ядром, но не одобренный моей проверкой.
        //
        // КОРОТКИЙ ОТКАТ СТАВИМ СРАЗУ, А НЕ ПОСЛЕ РАЗГОВОРА (Кодекс, второй проход).
        // Десятиминутный запрет ставится в разговоре, то есть только если спутник дошёл;
        // сорвавшийся ПЕШИЙ путь закрывает себя сам (см. «больше не пробую»). Но между
        // выбором и разговором цель может погибнуть, исчезнуть или спутника втянет в бой —
        // тогда записи не остаётся ни от кого, и следующий осмотр выберет ту же особь
        // снова. Для одобренной цели это терпимо: её выбрал не знак, а проверка. Для
        // запасной — нет, она выбрана ИМЕННО потому, что проверка её отвергла.
        //
        // Две минуты, а не десять: если разговор состоится и квест возьмётся, откат
        // снимается там же — у Макбрайда тридцать квестов, и возвращаться к нему надо
        // сразу, а не через десять минут.
        if (!best && marked)
        {
            best = marked;
            c.GiverUnreachable[marked->GetGUID()] = 120000;
        }
        if (!c.GiverDiagDone)
        {
...
private:
    ...
    // РЕЗЕРВ ТОЧКИ УМИРАЕТ ВМЕСТЕ СО СПУТНИКОМ (Кодекс, задачи 147 и 148).
    //
    // Вынесено отдельно, потому что вызывать это надо на ОБОИХ путях выхода — и когда сессия
    // есть, и когда её уже нет. Первая редакция стояла после раннего возврата `if (!c.Session)`
    // и потому пропускала ровно тот случай, ради которого писалась: спутник без сессии уносил
    // резерв с собой навсегда.
    void ReleaseLockHold(Companion& c)
    {
        if (!c.LockCastSpawn)
            return;
        if (auto hold = _lockCastHold.find(c.LockCastSpawn); hold != _lockCastHold.end()
            && hold->second.first == c.Guid.GetCounter())
            _lockCastHold.erase(hold);
        c.LockCastSpawn = 0;
        c.LockCastSpellId = 0;
        c.LockCastId.Clear();
    ...
    void DropSession(Companion& c)
    {
        ...
        // (Кодекс, задача 149: «освобождение без доказательства, что владелец вышел из мира»).
        //
        // Указатель на сессию в модуле ЗАВОДИТСЯ ровно в одном месте — при создании сессии
        // спутника, — и ОБНУЛЯЕТСЯ тоже ровно в одном: в конце этой самой функции, уже ПОСЛЕ
        // `LogoutPlayer` и `ReleaseLockHold`. Третьего места нет.
        //
        // Отсюда состояние «сессия пуста, а резерв ещё за нами» недостижимо: либо спутник
        // сессии никогда не имел и, стало быть, ничего не кастовал, либо он уже прошёл здесь
        // раньше — и тогда резерв снят, а `LockCastSpawn` обнулён до обнуления сессии. Вызов
        // ниже остаётся как страховка на случай будущей третьей точки обнуления и сегодня
        ...
        // ОТПУСКАЕМ ПОСЛЕ ВЫХОДА ИЗ МИРА, А НЕ ДО НЕГО (Кодекс, задача 148, пункт 3).
        //
        // Освобождение стояло раньше `LogoutPlayer`, и между ними оставалось окно, в котором
        // точка уже свободна, а заклинание прежнего владельца ещё живо — то самое пересечение,
        // против которого весь резерв и заводился. `LogoutPlayer` снимает персонажа с карты, а
    ...
    // КТО СЕЙЧАС ЧИТАЕТ ЗАКЛИНАНИЕ ЗАМКА НА ЭТОЙ ТОЧКЕ: спавн -> (низкая часть GUID, начало).
    //
    // Проверка доступности объекта отвечает на вопрос «свободен ли он СЕЙЧАС», а каст длится.
    // Два спутника успевают пройти её оба до того, как первый откроет дверь, и тогда второй
    // примет чужой переход состояния за своё применение (Кодекс, задача 139). Помеха здесь не
    // посторонний игрок, а свой же напарник, и их на обходе восемь — поэтому ошибка не
    // единична, а повторяется на каждой точке и может съесть весь обход.
    //
