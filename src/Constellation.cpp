...
// ОБЁРТКА ДЛЯ ЛЕСТНИЦЫ: подпись прежняя, поэтому ни одно место вызова не тронуто.
    ...
    void BehaveTick(Companion& c, uint32 diff)
    {
        ...
        if (!self->IsAlive())
        {
            ...
            if (!c.DeathCounted)
            {
                ...
                // прибор: зона, место, и три ближайших враждебных с их уровнями.
                {
                    ...
                    // С ЧЕМ ВХОДИЛИ В ЭТОТ БОЙ (0026 п. 1б, 2026-09-15): заступников по снимку обхода
        ...
        // такт движка; из Idle остались таймер обзора и подбор экипировки. `Mode` живёт как имя
        // для журнала: он всегда «стою», и переходы через `Switch` печатают только события
        // сервисов (подъём, гибель, поломка).
        ...
        // ДВИЖОК РЕШАЕТ — И ЭТО ЕДИНСТВЕННОЕ МЕСТО, ГДЕ РЕШАЮТ. Выше: дроссель `idleScan`,
        // который обязан тикать всегда, и обновки, которые ничего не переключают.
        //
        // ВЫКЛЮЧЕНИЕ ЗАБЫВАЕТ, А НЕ ЗАМОРАЖИВАЕТ. Снятый флаг останавливает такт и сам по себе
        // не трогает состояние; `HasTicked` показывает, что состояние заводилось, и обнуляется
    ...
    //   Won:     `KillSpots[клетка]++` с потолком 60000 — клетка ПОБЕДИТЕЛЯ, как в `Attacking`
    //            (`:5569-5578`). `DeathAt` победа БОЛЬШЕ НЕ ЧИСТИТ (2026-09-14, 0026 п. 1): у
    //            лестницы стояло «выиграл — значит зона по нему», но в толпе спутник убивает
    //            одного и гибнет от остальных — счётчик сбрасывался на каждой такой победе, и
    //            ворота «трижды за десять минут → целительница» (`:1889`) открывались 4 раза на
    //            55 гибелей. Замер четырёх часов: после подъёма у тела 22 % гибнут за минуту,
    //            после целительницы — 0 из 27. Окно в десять минут (`:2682`) забывает само.
