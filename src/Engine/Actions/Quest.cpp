...
namespace
{
    ...
    public:
        ...
        // Ядро само решает, достаточно ли близко (`CanInteractWithQuestGiver`), но искать
        // существо по всей карте незачем: за этим порогом ответ всё равно будет «нет».
        // Порог виден и `Possible`, и `Execute` — они обязаны спрашивать одно и то же.
        static constexpr float INTERACTION_RANGE_YARDS = 8.0f;
        ...
        bool Possible(Ctx& ctx, Bid const& bid) override
        {
            ...
            for (TurnInCandidate const& t : Val<ValueId::CompletedTurnIns>(ctx))
            {
                ...
                if (!t.EnderEntry)
                    return true;
                return ctx.World.NearestCreature(t.EnderEntry, INTERACTION_RANGE_YARDS).has_value();
            }
            return false;
        }
        ...
        bool Execute(Ctx& ctx, Bid const& bid) override
        {
            ...
            for (TurnInCandidate const& t : Val<ValueId::CompletedTurnIns>(ctx))
            {
                ...
                // БЛИЖАЙШИЙ ПО ВИДУ — НЕ ДОКАЗАТЕЛЬСТВО, И ЭТО НАМЕРЕННО. Поиск фильтрует
                // только вид и жизнь; годность принимающего решает ядро в обработчике, куда
                // дверь и отправляет пакет. Дублировать его проверки здесь значило бы завести
                // вторую копию правил, которая разойдётся с первой (Кодекс, п. 6).
                std::optional<ObjectGuid> const ender =
                    ctx.World.NearestCreature(t.EnderEntry, INTERACTION_RANGE_YARDS);
                if (!ender)
                    return false;
