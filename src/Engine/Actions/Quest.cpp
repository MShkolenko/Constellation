...
inline constexpr float TURNIN_TALK_YARDS    = 4.0f;
    ...
    public:
        ...
        void DefaultBids(Ctx& ctx, BidSink& sink) const override
        {
            ...
            // походов. REL_NORMAL, как бой: на равной цене решает расстояние. НАЗВАННОЕ
            // РАСХОЖДЕНИЕ: у лестницы бой всегда раньше сбора (`:3250` против `:3314`); здесь
            // ближайшая цель — будь то существо или сундук — берётся первой. Клетка и разговор
            // (`Fight.cpp`) остаются выше по той же цене расстояния. ВОРОТА ОТДЫХА — те же, что
            // у боя перед ставками по обзору: `Idle` лестницы отдыхает раньше всякого отбора, и
            // спутник на двадцати процентах не должен выбирать близкий сундук вместо отдыха
            // (Кодекс, задача mtzy1m8a, п. 5).
            if (!ctx.World.IsInCombat() && !RestWanted(ctx))
            {
                GatherSpot const& gs = Val<ValueId::GatherTarget>(ctx);
                if (gs.SpawnId && gs.MapId == ctx.World.MapId())
                    sink.Add(ActionId::GatherObjective, REL_NORMAL, Subject::OfSpawn(gs.SpawnId));
            }
