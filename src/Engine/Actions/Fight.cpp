...
namespace
{
    ...
    public:
        ...
        void DefaultBids(Ctx& ctx, BidSink& sink) const override
        {
            ...
            // Поэтому ставка на цель из обхода НЕ СТАВИТСЯ, пока отдых хочется и мы не в бою;
            // вступивший бой выше не трогается — он ставится из состояния, как и у лестницы.
            // (Первая редакция этого комментария объясняла гарду «ничьёй, которую решала соль» по
            // ценам 19.4–19.7 против 20.00 — это были строки такта ВЫХОДА лестницы из отдыха, а не
            // ничья; поправка Мастера 19:40. Гарда остаётся как перенос порядка, не как правка числа.)
            ...
            // бой только цели задания (`Constellation.cpp:12249-12251`), а ветка `Attacking`
            // лестницы бьёт `GetVictim()` — любого (`:5565`). Когда лестница дерётся с тем, кого
            // обход отверг, у движка боевой ставки нет — и без этих двух строк была бы разговорная
            // или клеточная, которая взяла бы ход и увела спутника из-под ударов. Без ставки движок
            // возвращает `Idle`, и режим лестницы идёт как шёл. Тень этого показать не может: в
            // тени ход никогда не берётся.
            if (!inCombat)
            {
                // КЛЕТКА СТАВИТСЯ НЕЗАВИСИМО ОТ БОЯ ЗАДАНИЯ, и спорить им не о чем. У лестницы
                // клетка достижима только когда бить некого; у движка это выходит само — существо,
                // невосприимчивое к игрокам, обход в боевые кандидаты не берёт.
                if (scan.CageSpawn)
                    sink.Add(ActionId::OpenCageForTarget, REL_HIGH,
                             Subject::OfSpawn(uint32(scan.CageSpawn)));

                // БИТЬ НЕКОГО, НО МОЖЕТ БЫТЬ ЕСТЬ С КЕМ ПОГОВОРИТЬ (`Constellation.cpp:3285`): выше
                // боя намеренно не ставим — REL_NORMAL против REL_HIGH; ниже сдачи по построению. С
                // взятием квеста — на одном уровне, порядок решает расстояние; лестница ставит
                // взятие выше (`:3262` против `:3291`), и разницу мерит тень.
