    ...
    bool Engine::Push(EngineState& st, std::vector<Bid> const& bids, float forced, uint32 nowMs)
    {
        ...
        for (Bid const& b : bids)
        {
            ...
            for (Bid& e : st.Queue)
            {
                ...
                // ФЛАГ «ПРЕДПОСЫЛКИ УЖЕ СПРОШЕНЫ» — У ВХОДЯЩЕЙ СТАВКИ, НЕ У ЛЕЖАЩЕЙ (замер
                // 2026-09-14 00:13-00:30, первый живой полёт). Переочередённая копия дороги
                // (`SkipPrerequisites`, `:1303`) переживала такт, свежая ставка стратегии
                // сливалась в неё и наследовала флаг — предпосылку (полёт) больше никто не
                // спрашивал: «лечу» один такт, потом дорога пешком, план висит. Свежая ставка
                // (false) — спросить снова; переочередённая (true) — пропустить в этом такте.
                e.SkipPrerequisites = b.SkipPrerequisites;
                merged = true;
                break;
            }
            if (merged)
