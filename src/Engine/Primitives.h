...
#define CONSTELLATION_BACKOFFS(X)                                                     \
    ...
    /* ОСОБЬ НЕ ПО УСЛОВИЯМ ЗАКЛИНАНИЯ — временно (лестница: TalkRetry, 60 с).              */ \
    ...
    /* ОБЪЕКТ — СВОЁ ПРОСТРАНСТВО КЛЮЧЕЙ. Точки появления объектов и существ нумеруются       */ \
    /* независимо, и могут совпасть; общий вид отсрочки на `Subject::OfSpawn` позволил бы     */ \
    /* неудаче у клетки молча запретить квестодателя (разбор). Ключ включает вид — этого      */ \
    /* довольно, чтобы пространства разошлись.                                                */ \
    X(ObjectUnreachable, "до объекта не добраться")
...
#undef CONSTELLATION_BACKOFF_NAME
    ...
    //
    // У лестницы этот же код лежит ТРИЖДЫ (`Constellation.cpp:3375`, `:3508`, `:3713`) — по копии
    // на каждую ходячую ветку. Здесь он один, и это единственная причина написать его ДО первого
    // ходячего действия, а не внутри него.
    //
    // ЧТО ОН РЕШАЕТ: идти дальше или бросить. Без него действие, идущее к цели, не может узнать,
    // что упёрлось, — и будет идти в стену, пока цель не пересчитается в ту же самую.
    // -------------------------------------------------------------------------------------------

    // ПРИБЛИЖЕНИЕ СЧИТАЕТСЯ С ЗАПАСОМ В ЯРД. Без запаса дрожание позиции на месте выглядело бы
    // как прогресс, и «не приближаемся» не наступило бы никогда.
    inline constexpr float  WALK_PROGRESS_YARDS = 1.0f;

    // ПОЛМИНУТЫ БЕЗ ПРИБЛИЖЕНИЯ — ЭТО НЕ «МЕДЛЕННО», А «НЕ ТУДА». Число у лестницы, не моё.
    inline constexpr uint32 WALK_NO_PROGRESS_MS = 30000;

    // ПОЧЕМУ БРОСИЛИ — ТРИ РАЗНЫЕ ПРИЧИНЫ, И ИХ НЕЛЬЗЯ СЛИВАТЬ. `Stalled` говорит движок
    // движения: физически упёрлись. `NoProgress` — идём, но не приближаемся: обход, запертая
    // дверь, чужой ярус. `TooLong` — дорога, которая не кончится. Разные причины требуют разных
    // отсрочек, и слитый в одну «не дошёл» ответ отнял бы у вызывающего это различие.
    enum class WalkVerdict : uint8
    {
        Going,          // приближаемся — продолжать
        Stalled,        // упёрлись
        NoProgress,     // полминуты не приближаемся
        TooLong,        // потолок по времени
    };

    inline char const* NameOf(WalkVerdict v)
    {
        switch (v)
        {
            case WalkVerdict::Going:      return "идём";
            case WalkVerdict::Stalled:    return "упёрлись";
            case WalkVerdict::NoProgress: return "полминуты без приближения";
            case WalkVerdict::TooLong:    return "потолок по времени";
            default:                      return "?";
        }
    }

    // Состояние одной дороги. Живёт в `EngineState`: у спутника в каждый момент одна цель, и
    // сбрасывается оно сменой цели, а не таймером.
    struct WalkProgress
    {
        // ПРИЗНАК «ЕЩЁ НЕ ШЛИ» — ФЛАГ, А НЕ ЧАСОВОЕ ЧИСЛО. У лестницы здесь `1.0e9f`, и внутри
        // одной функции это безобидно; в общем примитиве это неявная зависимость от того, каких
        // расстояний «не бывает» (Кодекс, пункт 8). Флаг не требует знать диапазон координат.
        bool   Measured  = false;
        float  Best      = 0.0f;        // лучшее расстояние, которого достигали к этой точке
        uint32 StuckMs   = 0;           // сколько подряд не приближаемся
        uint32 StartedMs = 0;           // когда вышли — для потолка по времени
        // ПРОДВИЖЕНИЕ ПО ПОСТРОЕННОМУ ПУТИ — ТОЖЕ ПРОГРЕСС (2026-09-14). По прямой городской путь
        // сначала УВОДИТ от цели: после посадки в Штормграде расстояние до Osric росло 494 -> 696,
        // и через тридцать секунд «без приближения» дорога сдавалась. Рост `WaypointIndex` того
        // же пути обнуляет счёт; перестроение пути (индекс назад к нулю) прогрессом не считается.
        size_t LastWaypoint = 0;
        bool   Regressed    = false;    // путь перестроен (индекс упал): следующий рост — не прогресс (Кодекс)

        // К ЧЕМУ идём. Смена ПРЕДМЕТА — новая дорога; смена ПОЛОЖЕНИЯ того же предмета — нет, и
        // это намеренно: потолок отмеряется на дорогу к цели, а не к координате, иначе
        // патрулирующий NPC обнулял бы его каждым своим шагом.
        Subject Toward;

        void Restart(Subject const& to, uint32 nowMs)
        {
            Measured  = false;
            Best      = 0.0f;
            StuckMs   = 0;
