...
namespace Constellation::Ai
{
    ...
    struct EngineState
    {
        ...
        // ПРИБОР, ЖИВУЩИЙ ДО ОТВЕТА. Один раз на спутника сказать, почему ставки на сдачу не
        // было в тот момент, когда лестница сдавала. Уходит вместе с разобранным расхождением —
        // и это записано здесь, чтобы его не приняли за постоянное состояние.
        bool TurnInGapLogged = false;
